...
| 2. Construcción y verificación de funcionamiento | [8. Implementación](08-implementacion.md), [9. Pruebas y verificación](09-pruebas-y-verificacion.md), [10. Resultados](10-resultados.md) | 289 pruebas, ocho recorridos Playwright, 35 rutas OpenAPI con esquema en cada 2xx; pruebas en `apps/api/test/`, `packages/*/src/*.test.ts` y `apps/web/e2e/`; capturas en `docs/evidence/fase-5/capturas/`. | PENDIENTE: Gemini real, OAuth real e instalación Android física dependen de tercero, credenciales o dispositivo; véase [Pendientes externos](PENDIENTES-EXTERNOS.md). |
