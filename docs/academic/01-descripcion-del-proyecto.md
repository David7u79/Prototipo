...
La fase verificable construye la base técnica de identidad, sesión, perfil deportivo, distribución controlada de una APK Android y una frontera para inteligencia artificial. El repositorio contiene una landing pública, una aplicación web, una aplicación móvil y una API central. El flujo disponible permite pasar de la landing a la web, registrarse o iniciar sesión, y crear o editar un perfil. En móvil incluye autenticación, restauración de sesión, inicio y perfil.

## 1.2 Alcance realizado

La API permite registro e inicio de sesión local, renovación y cierre de sesión. Cuando existe configuración del servidor, acepta un ID token de Google y verifica la identidad fuera del cliente. Una sesión autenticada permite crear, consultar y actualizar un perfil con nombre visible, nivel de experiencia y objetivo principal. Estas responsabilidades se encuentran en `apps/api/src/auth` y `apps/api/src/profile`.
...
La primera fase no implementa catálogo de movimientos, captura de entrenamientos, resultados, marcas personales, historial, estadísticas, evolución ni asistente de IA. La interfaz puede contener pantallas de orientación para algunos de esos temas, pero no existe el dominio ni la persistencia correspondiente. Por ello no se presentan como funcionalidades terminadas ni como resultados de uso.
